...
def get_combined_result(selected_counts, indicator):
	"""선택된 물질의 개수를 보고 지시약별로 결과 색과 설명을 반환합니다."""
	acid = 0
	base = 0
	neutral = 0
	...
	if acid > base and acid > neutral:
	...
	elif base > acid and base > neutral:
		kind = "base"
		page_type = "염기성"
		explanation = "염기성 물질이 더 많아서 염기성 쪽 색이 나와요."
	elif acid == base and acid > 0:
		# 산성과 염기성이 같을 때는 중성(또는 혼합)으로 처리
		kind = "neutral"
		page_type = "중성에 가까운 혼합물"
		explanation = "산성과 염기성이 비슷해서 중간색이 돼요."
	elif neutral >= acid and neutral >= base:
		kind = "neutral"
		page_type = "중성"
		...
		kind = "neutral"
		page_type = "중성"
		explanation = "여러 물질이 섞여서 중성에 가까운 색이 나올 수 있어요."

	# 지시약별 색 정보 사용
	color_info = indicator_options.get(indicator, {}).get(kind)
	if not color_info:
		# 안전한 기본값
		color_info = {"label": "보라색 또는 초록색", "css": "#8e44ad"}
...
if st.session_state.started:
	...
	if selected_names:
		...
		combined = get_combined_result(selected_counts, indicator)
